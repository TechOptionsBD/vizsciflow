...
class Related(object):
    ...
    def __get__(self, instance, owner):
        ...
        return instance.__ogm__.related(self.direction, self.relationship_type, cls, self.order_by)
...
class OGM(object):
    ...
    def related(self, direction, relationship_type, related_class, order_by):
        """ Return :class:`.RelatedObjects` for given criteria.
        """
        key = (direction, relationship_type)
        if key not in self._related:
            self._related[key] = RelatedObjects(self.node, direction, relationship_type, related_class, order_by)
