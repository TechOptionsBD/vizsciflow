function monitorGraphViewModel()
        {
			var self = this;
			var monitorDiagram;
    		...
    		self.show = function(rels) {
				...
				monitorDiagram.grid.visible = true;
					

				// create the Overview and initialize it to show the main Diagram
				var monitorOverview =
				$$(go.Overview, "monitorDiagramOverview",
					{ observed: monitorDiagram });
				monitorOverview.grid.visible = true;


				monitorDiagram.nodeTemplate = $$(go.Node, "Spot",
				...
				monitorDiagram.model = go.Model.fromJson(JSON.stringify(provModelData));  //load()
				
				monitorDiagram.model.isReadOnly = true;
			}

			self.requestUpdate = function(){
				monitorDiagram.requestUpdate();
			}	
		}
