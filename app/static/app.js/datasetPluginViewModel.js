function DatasetPluginViewModel() {
  var self = this;
  self.scidatamgrURI = 'http://localhost:8080'
  ...
  self.renderDataset = function() {
    ...
    // fetching api data
    ajaxcalls.simple(self.scidatamgrURI + '/api/plugin/datasets', 'GET', { secret_key: '123456789', username: 'C M Khaled Saifullah', pageNum: '1', numOfItems: '10' }).done(function(dataset) {    
  ...
  const onItemExpand = function (e, dataset) {
    ajaxcalls.simple(self.scidatamgrURI + `/api/plugin/dataset/${dataset.id}/data`, 'GET', { secret_key: '123456789', username: 'C M Khaled Saifullah' }).done(function(data) {    
      if (data === undefined)
        return;
