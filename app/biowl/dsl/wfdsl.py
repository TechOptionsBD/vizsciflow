...
class Module(object):
    ...
    def compare(module1, module2, deep = False):
        ...
        json = module_compare_json(node1, node2)
        return json
        
        
class Run(object):
    def __init__(self, id = None, runItem = None):
        result = RunnableItem.load(id = id) if id else runItem
        self._node = result
    
    @staticmethod
    def get(RunnableItem):
        try:        
            runItems = RunnableItem
            if not isinstance(runItems, list):
                result = Run(runItem = runItems)
                return result
    ...
    def compare(run1, run2, deep = False):
        ...
        json = run_compare_json(node1, node2)
        if deep:
            from2 = []
            if run1:
...
def run_compare_json(node1, node2):
...
def module_compare_json(node1, node2):
