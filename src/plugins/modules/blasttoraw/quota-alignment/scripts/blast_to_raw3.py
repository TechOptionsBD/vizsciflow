...
def main(blast_file, options):
    ...
    if is_self:
        print("... looks like a self-self BLAST to me", file=sys.stderr)
    ...
    print("read annotation files %s and %s" % (qbed_file, sbed_file), file=sys.stderr)
    qbed = Bed(qbed_file)
    sbed = Bed(sbed_file)
    ...
    fp = open(blast_file)
    print("read BLAST file %s (total %d lines)" % \
            (blast_file, sum(1 for line in fp)), file=sys.stderr)
    fp.seek(0)
...
#             key=lambda b: b.score, reverse=True)
    ...
    for b in blasts:
        ...
        if query not in qorder:
            print("WARNING: %s not in %s" % (query, qbed.filename), file=sys.stderr)
            continue
        if subject not in sorder:
            print("WARNING: %s not in %s" % (subject, sbed.filename), file=sys.stderr)
            continue
    ...
    if global_density_ratio:
        print("running the global_density filter" + \
                "(global_density_ratio=%d)..." % options.global_density_ratio, file=sys.stderr)
        ...
        print("after filter (%d->%d)..." % (before_filter, len(filtered_blasts)), file=sys.stderr)

    if tandem_Nmax:
        print("running the local dups filter (tandem_Nmax=%d)..." % tandem_Nmax, file=sys.stderr)
        ...
        print("after filter (%d->%d)..." % \
                (before_filter, len(filtered_blasts)), file=sys.stderr)
    ...
    if filter_repeats:
        before_filter = len(filtered_blasts)
        print("running the repeat filter", end=' ', file=sys.stderr) 
        filtered_blasts = list(filter_repeat(filtered_blasts))
        print("after filter (%d->%d)..." % (before_filter, len(filtered_blasts)), file=sys.stderr)

    if cscore:
        before_filter = len(filtered_blasts)
        print("running the cscore filter (cscore>=%.2f)..." % cscore, file=sys.stderr)
...
def write_localdups(dups_fh, tandems, bed):
    ...
    if dups_fh:
        print("write local dups to file", dups_fh.name, file=sys.stderr)
    
    print("#Local duplications file", file=dups_fh)
    tandem_groups = []
...
def write_new_bed(bed, children):
    ...
    print("write tandem-filtered bed file %s" % out_name, file=sys.stderr)
    fh = open(out_name, "w")
...
def write_raw(qorder, sorder, filtered_blasts, raw_fh):
    ...
    print("write raw file %s" % raw_fh.name, file=sys.stderr)
    for b in filtered_blasts: 
        ...
        print("\t".join(map(str, (qseqid, qi, sseqid, si, score))), file=raw_fh)
...
def filter_to_global_density(blast_list, gene_count, global_density_ratio):
    ...
    print("cutting at:", max_hits, file=sys.stderr)
...
def filter_repeat(blast_list, evalue_cutoff=.05):
    ...
    print("(expected_count=%d)..." % expected_count, file=sys.stderr)
