...
@depends
def run_lastdb(infile=None, outfile=None, lastdb_bin="/home/vizsciflow/src/plugins/modules/last/bin/lastdb"):
...
def main(args):
    ...
    # Chi: I hardcoded these paths because I got file not found errors, but is there a better way to do this?
    # Also hardcoded default path to lastdb on line no. 25
    
    # getpath = lambda x: op.join(path, x) if path else x
    lastdb_bin = "/home/vizsciflow/src/plugins/modules/last/bin/lastdb" #path.join(getpath, "lastdb")
    lastal_bin = "/home/vizsciflow/src/plugins/modules/last/bin/lastal"#path.join(getpath, "lastal")
    lastex_bin = "/home/vizsciflow/src/plugins/modules/last/bin/lastex"#path.join(getpath, "lastex")
