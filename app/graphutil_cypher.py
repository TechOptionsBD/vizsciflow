...
# 
#     id = Property(123)
#     properties = Property("xyz")    
    
    def __init__(self, **kwargs):
        self._id = None # this will be set when the item is created or updated from neo4j
         
        self._created_on = Property("created_on", neotime.DateTime.utc_now())
        self._modified_on = Property("modified_on", neotime.DateTime.utc_now())
...
class RunnableItem(NodeItem): #number1
    __celery_id = Property("celery_id", 0)
    def __init__(self, user_id, workflow_id, script, arguments, **kwargs):
        super().__init__(**kwargs)
        
        # RDBMS info
        self._user_id = Property("user_id", user_id)
        self.__workflow_id = Property("workflow_id", workflow_id)
        
        self._name = Property("name", Workflow.query.get(workflow_id).name)
        
        self._out = Property("out", "")
        self._error = Property("error", "")
        
        self._script = Property("script", script)
        self._status = Property("status", Status.RECEIVED)
        self.__duration = Property("duration", 0)
        self.__arguments = Property("arguments", arguments)
    ...
    def load(run_id = None, workflow_id = None):
        if run_id:
            runnableMatcher = RunnableItem.match(graph(), run_id)
            r = runnableMatcher.first()
            return r
            #return RunnableItem.load_from_node(load_node(run_id))
        elif workflow_id:
            runnableMatcher = RunnableItem.match(graph()).where("._workflow_id = {0}".format(workflow_id))
            return [r._object_class for r in runnableMatcher]
            #return RunnableItem.load_runnables_from_cypher("MATCH(n:Run) WHERE n.workflow_id = {0} RETURN n".format(int(workflow_id)))
        else:
            runnableMatcher = RunnableItem.match(graph())
            return [r._object_class for r in runnableMatcher]
            #return RunnableItem.load_runnables_from_cypher("MATCH(n:Run) RETURN n")
    
    @staticmethod
    def load_for_users(user_id):
        runnableMatcher = RunnableItem.match(graph()).where("._user_id = {0}".format(user_id))
        return [r._object_class for r in runnableMatcher]
    ...
    def create(user_id, workflow_id, script, args):
        try:
            ...
            graph().push(item)
            item.id = item._GraphObject__ogm.node.identity
