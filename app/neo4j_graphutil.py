...
class NodeItem():
    ...
    def add_new_property(self, name, value):
        self.__class__ = type(
            type(self).__name__, (self.__class__,), {name: ""}
        )
        setattr(self, name, value)
    
    @staticmethod
    def create_and_query(**kwargs):

        # return items with id directly
        id = kwargs.pop('id', None)
        query = ""
        if id is not None:
            query = "ID(n)={0}".format(id)
        ...
        labelq = "n:{0}".format(label) if label else "n"
        if query:
            query = "WHERE " + query
        return graph().read_transaction(lambda tx: list(tx.run("MATCH ({0}) {1} RETURN n".format(labelq, query))))

    @staticmethod
    def get_all_with_and(cls_, **kwargs):
        results = NodeItem.create_and_query(label=cls_.label, **kwargs)
        items = []
        for r in results:
            item = cls_(**r['n'])
            item.id = r['n'].id
    ...
    @staticmethod
    def get_first_with_and(cls_, **kwargs):
        results = NodeItem.create_and_query(label=cls_.label, **kwargs)
        if not results:
            return None
        item = cls_(**results[0]['n'])
        item.id = results[0]['n'].id
        return item

    @staticmethod
    def create_or_query(**kwargs):
        # return items with id directly
        id = kwargs.pop('id', None)
        #query with other attributes
        query = ""
        ...
        results = graph().read_transaction(lambda tx: list(tx.run("MATCH (n:{0}) {1} RETURN n".format())))

    @staticmethod
    def get_all_with_or(cls_, **kwargs):
        results = NodeItem.create_or_query(**kwargs)    
        
        return [cls_.wrap(r) for r in results]

    @staticmethod
    def get_first_with_or(cls_, **kwargs):        
        results = graph().read_transaction(lambda tx: list(tx.run("MATCH (n:{0}) {1} RETURN n".format())))
        return [cls_.wrap(r) for r in results]
...
#     
    ...
    def load(self, node):
        self.id = node.get('id')
        self.created_on = node.get('created_on')
        self.modified_on = node.get('modified_on')
        return self

    def push(self, **kwargs):
        results = graph().write_transaction(lambda tx: list(tx.run("MATCH (n:{0}) {1} RETURN n".format())))
...
class DataSourceItem(NodeItem):
    ...
    def load(self, node):
        ...
        self.public = bool(node.get('public', True))
        self.user = node.get('user')
        self.password = node.get('password')
        self.prefix = node.get('prefix')
        self.active = bool(node.get('active', True))
        self.temp = bool(node.get('temp', False))
        return super(DataSourceItem, self).load(node)
    ...
    @staticmethod
    def get(**kwargs):
        return NodeItem.get_all_with_and(DataSourceItem, **kwargs)
...
class RoleItem(NodeItem):
    ...
    def load(self, node):
        self.name = node.get('name')
        self.default = bool(node.get('default', False))
        self.permissions = int(node.get('permissions', 0x00))
        return super(RoleItem, self).load(node)

    @staticmethod
    def get(**kwargs):
        return NodeItem.get_all_with_and(RoleItem, **kwargs)
...
class UserItem(NodeItem, UserMixin):    
    ...
    def load(self, node):
        ...
        self.password_hash = node.get('password_hash')
        self.confirmed = node.get('confirmed')
        self.name = node.get('name')
        self.location = node.get('location')
        self.about_me = node.get('about_me')
        self.member_since = node.get('member_since')
        self.last_seen = node.get('last_seen')
        self.avatr_hash = node.get('avatar_hash')
    ...
    def ping(self):
        ...
        self.push(last_seen=self.last_seen)
...
class ModuleItem(NodeItem):
    ...
    @staticmethod
    def add_user_access(self, name, package, user_id, access):
        user = UserItem.get(id=user_id)
        user.add_module_access(name, package, access)

    def add_user_access_to_self(self, user_id, access):
        user = UserItem.get(id=user_id)
        user.modules.add(self)

    @staticmethod
    def get(**kwargs):
        return NodeItem.get_all_with_and(ModuleItem, **kwargs)
...
class WorkflowItem(NodeItem):
    ...
    def add_module(self, **kwargs):
        module = NodeItem.get_first_with_and(ModuleItem, **kwargs)
        if module:
            self.modules.add(module)
            graph().push(self)
            return module
    ...
    @staticmethod
    def get(**kwargs):
        return NodeItem.get_first_with_and(WorkflowItem, **kwargs)
...
class RunnableItem(NodeItem): #number1
    ...
    started_on = neotime.DateTime.min
...
#         return [r for r in user.runs if not r.provenance]
    ...
    @staticmethod
    def get(**kwargs):
        return NodeItem.get_all_with_and(RunnableItem, **kwargs)
